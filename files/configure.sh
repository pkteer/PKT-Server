...
if [ -f /data/cjdroute.conf ]; then
    ...
    /server/cjdns/cjdroute --genconf | /server/cjdns/cjdroute --cleanconf > /server/cjdns/cjdroute.conf | jq '.interfaces.UDPInterface[0].bind = "0.0.0.0:'"$CJDNS_PORT"'"' cjdroute.conf | sponge cjdroute.conf
